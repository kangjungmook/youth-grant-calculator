 ...
 * 정책이 바뀌면 이 파일만 수정하세요. 화면/매칭 로직은 손댈 필요 없습니다.
 *
 * id            고유 키
 * name/agency   정책명 / 주관기관
 * icon          카드 이모지 (아이콘 컴포넌트로 교체 가능)
 * badge         강조 배지 (없으면 null) — 예: "10월 2차 신청"
 * benefit       결과 카드의 큰 예상 혜택 문구
 * summary       한 줄 요약
 * details       [자세히 보기] 펼침 항목 배열
 * monthly       월 현금성 지원액(원) → 합계 "매달 현금성 지원"
 * lump          만기/연간 목돈(원)   → 합계 "만기·연간 목돈"
 * cond          매칭 조건 (모두 AND, 필드가 없으면 제한 없음)
 *   minAge / maxAge      만 나이
 *   maxIncomeMan         월 세전 소득 상한(만원)
 *   residence            ['parents','alone'] 중 허용값
 *   status               ['sme','jobseek','student'] 중 허용값
 * missMsg       조건 미달 시 하단 안내 문구
 */
